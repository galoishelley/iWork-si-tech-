...
#endif
{
	...
	/**/
	if (cd = tpconnect(serverName ,NULL,0,TPSENDONLY|TPSIGRSTRT)==-1)
	{
		printf("tpconnect():%s \n",tpstrerror(tperrno));
		printf("tpconnect():ret=%d\n",cd );
		tpfree((char *)sendbuf);
		tpfree((char *)rcvbuf);
		tpterm();
		exit(1);
    ...
    reqlen=Fsizeof32(sendbuf);
    if (tpsend(cd, (char *)sendbuf, reqlen, TPRECVONLY, &revent )==-1)
	{
		printf("tpsend():%s\n",tpstrerror(tperrno));
	...
	while(1){
		if (tprecv(cd, (char **)&rcvbuf, &len, TPNOCHANGE, &revent)!=-1)
		{
			printf("tprecv():%s\n",tpstrerror(tperrno));
			printf("revent is %ld\n",revent);
			printf("TPEV_SVCSUCC is %ld\n",TPEV_SVCSUCC);
		}
		if(tperrno==TPEEVENT && revent==TPEV_SVCFAIL)
		{
			printf("revent error \n");
		...
		{
			printf("revent TPEV_SVCSUCC :%d\n",tperrno);
			break;
		}
		if(tperrno!=TPEEVENT )
		{
			printf("revent error :%d\n",tperrno);
			break;
		}
	    printf("++++ tpcall succ !!! ++++\n");
