...
#endif
{
        ...
        long reqlen;
        char serverName[32],buffer[2560];
	char *envp=NULL, temps[128];/*+lugz+*/

        if(argc < 4) {
                (void) fprintf(stderr, "Usage: cl32 serverName, SEND_PARMS_NUM RECP_PARMS_NUM, arg1, arg2,...\n");
                exit(1);
        }
	envp= tuxgetenv("WSNADDRONE");/*+lugz+*/
	if (envp == NULL)/*+lugz+*/
	{/*+lugz+*/
		printf("envp NULL\n");/*+lugz+*/
	}/*+lugz+*/
	...
	memset(temps, 0, sizeof(temps));/*+lugz+*/
	strcpy(temps, "WSNADDR=");/*+lugz+*/
	strcat(temps, envp);/*+lugz+*/
	printf("put env [%s]\n", temps);/*+lugz+*/
	
	if (tuxputenv(temps) != 0)/*+lugz+*/
	{/*+lugz+*/
		printf("tuxputenv error\n");/*+lugz+*/
	}/*+lugz+*/
	else/*+lugz+*/
	{/*+lugz+*/
		printf("tuxputenv succ\n");/*+lugz+*/
	}/*+lugz+*/
