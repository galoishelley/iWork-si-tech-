...
void PubLogicDeal(i_prog_label,i_db_source,i_db_dest,i_table_label,i_op_accept)
char *i_prog_label;
char *i_db_source;
char *i_db_dest;
char *i_table_label;
long i_op_accept;
{
	...
	sprintf(v_sql_buf,"SELECT order_accept,order_type,id_type,nvl(id_no,' '),nvl(interface_type,' '),nvl(busi_type,' '),\
			...
			FROM wOrderSend%s \
			WHERE op_accept=%ld AND order_type=any('1','2') ORDER BY order_accept",i_table_label,i_op_accept);
	...
	char **argv;
{
	...
	if (argc!=3){
		printf("Usage:singleOrder TableLabel OpAccept!\n");
		exit(0);
	}
printf("Begin %s %s %s\n",argv[0],argv[1],argv[2]);	
	init(v_table_label);
	strcpy(v_table_label,argv[1]);
	v_op_accept=atol(argv[2]);
	...
	PubLogicDeal(PROG_NAME_LABEL,g_PUB_Source,g_PUB_Dept,v_table_label,v_op_accept);
