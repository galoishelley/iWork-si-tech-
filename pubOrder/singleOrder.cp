...
void PubLogicDeal(i_prog_label,i_db_source,i_db_dest,i_table_label,i_order_accept)
char *i_prog_label;
char *i_db_source;
char *i_db_dest;
char *i_table_label;
long i_order_accept;
{
	...
	v_ret=readDBCfg(i_db_source);
	if (v_ret<0){
		errLog(i_prog_label,"Get DB Config Error!!\n");
		exit(-1);	
	}

	DEBUG(i_prog_label,"Begin connectDBA [%s][%ld]\n",i_table_label,i_order_accept);
	v_ret=connectDB(i_prog_label,PUB_DB_LABEL_Y,i_db_source);
	if(v_ret!=0){
		errLog(i_prog_label,"connectDBA Err\n");
		exit(0);
	}
		
	v_ret=readDBCfg(i_db_dest);
	if (v_ret<0){
		errLog(i_prog_label,"Get DB Config Error!!\n");
		exit(-1);	
	}
	
	DEBUG(i_prog_label,"Begin connectDBB [%s][%ld]\n",i_table_label,i_order_accept);
	v_ret=connectDB(i_prog_label,PUB_DB_LABEL_Y,i_db_dest);
	if(v_ret!=0){
		errLog(i_prog_label,"connectDBB Err\n");
		exit(0);
	}

	init(v_sql_buf);	init(v_order_data);

	DEBUG(i_prog_label,"Begin Select [%s][%ld]\n",i_table_label,i_order_accept);
	sprintf(v_sql_buf,"SELECT order_accept,order_type,id_type,nvl(id_no,' '),nvl(interface_type,' '),nvl(busi_type,' '),\
			...
			FROM wOrderSend%s \
			WHERE order_accept=%ld AND order_type=any('1','2') ORDER BY order_accept",i_table_label,i_order_accept);
	...
	EXEC SQL AT :i_db_source CLOSE SelectCur;
	DEBUG(i_prog_label,"End Select [%s][%ld] SQLNUM=[[%d]\n",i_table_label,i_order_accept,v_rownum);
	...
	DEBUG(i_prog_label,"Begin Insert [%s][%ld]\n",i_table_label,i_order_accept);
	init(v_sql_buf);
	...
	DEBUG(i_prog_label,"End Insert [%s][%ld]\n",i_table_label,i_order_accept);
	...
	DEBUG(i_prog_label,"End DataDet [%s][%ld]\n",i_table_label,i_order_accept);
	return;
}
	...
	char **argv;
{
	...
	if (argc!=3){
		printf("Usage:singleOrder TableLabel OrderAccept!\n");
		exit(0);
	}
printf("Begin %s %s %s\n",argv[0],argv[1],argv[2]);
	init(v_table_label);
	strcpy(v_table_label,argv[1]);
	v_order_accept=atol(argv[2]);
	...
	DEBUG(PROG_NAME_LABEL,"Begin PubLogicDeal [%s][%s]\n",argv[1],argv[2]);
	
	PubLogicDeal(PROG_NAME_LABEL,g_PUB_Source,g_PUB_Dept,v_table_label,v_order_accept);
	
	DEBUG(PROG_NAME_LABEL,"End PubLogicDeal [%s][%s]\n",argv[1],argv[2]);
printf("End   %s %s %s\n",argv[0],argv[1],argv[2]);
	exit(0);
}
