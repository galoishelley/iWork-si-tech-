...
/**********************************************
 @FUNC	 fDsmpPlatformOrder		 业务平台定购BOSS产品函数调用
...
int	fBossPlatformOrder(DSMPORDEROpr	*tBizData, char	*vEfftFlag,char	*retMsg)
{
	...
	EXEC SQL END DECLARE SECTION;
		...
		if (SQLCODE	!= 0)
		{
			...
			sprintf(retMsg,	"用户不存在或已销户!");
			printf("读取表dCustMsg出错[%s][%d]!",	phoneNo,SQLCODE);
			goto end_valid ;
		}
		Trim(idNo);
		Trim(vrun_code);
		...
		printf("\n\n测试 oprCode = %s bizType = %s",oprCode,bizType);
		...
		else if	(strncmp(oprCode,BIZ_OPR_CHGDAT,2) ==	0)
		{
			...
			strcpy(tBizData->send_eff_time		,opTime	 );
					...
					|| (strcmp(oprCode,"01") ==	0	&& strcmp(bizType,"41")==	0)/**WHL add for 移动微博	at 20111221	**/
					|| (strcmp(oprCode,"01") ==	0	&& strcmp(bizType,"43")==	0)/**shijing add for 便民卡	at 20120118	**/
					|| (strcmp(oprCode,"06") ==	0	&& strcmp(bizType,"73")==	0)/**Liu. add for 手机钱包	at 2013.01.23	**/
					|| (strcmp(oprCode,"26") ==	0))
		{
				/**06-订购 11-赠送 21-订购确认 01-注册 21-主动暂停(31)**/
				...
				printf("检查SP代码、SP业务代码返回retCode=[%s]\n"	,retCode);
				if (strcmp(retCode,	"000000")	!= 0)
				{
				...
				/**	用户信息开关功能检查 **/
				/**不判断梦网开关	 --zhangzhuo 20101019
				sprintf(retCode, "%06d", fDsmpInfoOnOffCheck(idNo, phoneNo,	bizType, spId, spBizCode,	belongCode,retMsg)); --	增加输入参数 by	zhangxina	@	20110328
				printf("用户信息开关功能检查retCode=[%s]\n"	,retCode);
				if (strcmp(retCode,	"000000")	!= 0)
				{
				...
				if (strncmp(oprCode,BIZ_OPR_11,2)	== 0)
				{
					/**	第三方用户信息开关功能检查 **/
					/**不判断梦网开关	 --zhangzhuo 20101019
					sprintf(retCode, "%06d", fDsmpInfoOnOffCheck(thirdIdNo,	thirdNo, bizType,spId, spBizCode,	belongCode,retMsg)); --	增加输入参数 by	zhangxina	@	20110328
					printf("第三方用户信息开关功能检查retCode=[%s]\n"	,retCode);
					if (strcmp(retCode,	"000000")	!= 0)
					{
						printf("fDsmpInfoOnOffCheck:retCode=[%s]	retMsg=[%s]	\n", retCode,retMsg	);
						goto end_valid ;
		...
		if((strcmp(oprCode,"26") !=	0))
			...
			if(strcmp(oprCode,"26")	== 0)/**续订 订购关系判断：**/
			{
					EXEC SQL select	count(*)
								 into	:icount
								 from	ddsmpordermsg
					...
					if(SQLCODE!=0)
					{
						...
						sprintf(retMsg,	"获取续订关系出错spid=[%s],bizcode=[%s],id_no=[%s],[%d]!", spId,spBizCode,idNo,SQLCODE);
						goto end_valid ;
					}
					if(icount==0)
					{
						strcpy(retCode,"111111");
						sprintf(retMsg,	"用户没有订购关系：spid=[%s],bizcode=[%s],id_no=[%s],[%d]!", spId,spBizCode,idNo,SQLCODE);
						goto end_valid ;
					}
			}
		}
		...
		)/*shijing add for 便民卡 at 20120118*/
		{
			/**99-DSMP全业务退订,02-用户注销**/
			Sinitn(temp_sql);
			iRet = 0;
			printf("查询业务平台代码temp_portalcode=[%s]\n",temp_portalcode);
			...
			strcpy(temp_order_flag				 ,"0"	 );
					 ...
					 ||((strcmp(oprCode,"02")	== 0)	&& (strcmp(bizType,"41") ==	0	)) /** WHL add for 移动微博	at 20111221	**/
					 ||((strcmp(oprCode,"02")	== 0)	&& (strcmp(bizType,"43") ==	0	))/** shijing add for 便民卡	at 201210118	**/
					 ...
					 ||((strcmp(oprCode,"11")	== 0)	&& (strcmp(bizType,"93") ==	0	)))
		{
			...
			#ifdef _DEBUG_
				printf("07-退订		tBizData->order_id = [%ld]\n", tBizData->order_id);
				printf("07-退订		tBizData->bizcode	=	[%s]\n", tBizData->bizcode);
			#endif
			strcpy(temp_order_flag				 ,"0"	 );
					 ...
					 ||	(strncmp(oprCode,BIZ_OPR_PWDRESET,2) ==	0))
		{
			...
			#ifdef _DEBUG_
				printf("密码修改	 tBizData->order_id	=	[%ld]\n",	tBizData->order_id);
				printf("密码修改	 oprCode = [%s]\n",	oprCode);
				printf("密码修改	 old_password	=	[%s]\n", old_password);
				printf("密码修改	 tBizData->password	=	[%s]\n", tBizData->password);
			#endif
		...
		else if(strncmp(temp_orderadd_flag,"1",1)	== 0)
		{
			...
			printf("单独处理 资料信息修改等	 仅仅涉及附加信息变更fPubDsmpOrderMsgAdd:retCode=[%s]\n" ,retCode);
			if (strcmp(retCode,	"000000")	!= 0)
			{
		...
		else if	(( ((strncmp(oprCode,BIZ_OPR_DEST,2) ==	0) &&	(strcmp(tBizData->serv_code, "41") !=	0))	/**	WHL	add	for	移动微博 at	20111221 **/
			...
			&& (strcmp(temp_reconfirm_flag,	"1") !=	0) )/*add by shijing for 便民卡 at 20120131*/
			{

				#ifdef _DEBUG_
						printf("生成产品定购关系	 tBizData->order_id	=	[%ld]\n",	tBizData->order_id);
						printf("生成产品定购关系	 lLoginAccept	=	[%ld]\n",	lLoginAccept);
						printf("生成产品定购关系	 temp_reconfirm_flag = [%s]\n",	temp_reconfirm_flag);
						printf("生成产品定购关系	 oprCode = [%s]\n",	oprCode);
						printf("生成产品定购关系	 tBizData->eff_time	 = [%s]\n",	tBizData->eff_time );
						printf("生成产品定购关系	 tBizData->end_time	 = [%s]\n",	tBizData->end_time );
				#endif
				/**	附加信息的处理 ***/
				if (strncmp(temp_orderadd_flag,"0",1)	== 0)
				{
					sprintf(retCode, "%06d", fPubDsmpOrderMsgAdd(tBizData,temp_order_flag,temp_orderadd_flag,retMsg));
					printf("附加信息的处理fPubDsmpOrderMsgAdd:retCode=[%s]\n"	,retCode);
					if (strcmp(retCode,	"000000")	!= 0)
					{
						printf("fPubDsmpOrderMsgAdd:retCode=[%s]	retMsg=[%s]	\n", retCode,retMsg	);
						goto end_valid ;
			...
			if (strncmp(oprCode,BIZ_OPR_89,2)	== 0)
			{
				...
				printf("发送到同步接口的信息表 fPubDsmpSubInfo:	retCode=[%s]\n"	,retCode);
				if (strcmp(retCode,	"000000")	!= 0)
				{
				...
				while(SQLCODE	== 0)
				{
					...
					printf("读取用户的产品定购信息fDsmpGetOrderInfo: retCode=[%s]\n" ,retCode);
					if (strcmp(retCode,	"000000")	!= 0)
					{
						printf("fDsmpGetOrderInfo:retCode=[%s]	retMsg=[%s]	\n", retCode,retMsg	);
						strcpy(retCode,"001305");
					...
					printf("22222222222222222222读取用户的产品定购信息fDsmpGetOrderInfo: retCode=[%s]\n" ,retCode);
					...
					strcpy(tBizData->end_time				,opTime	);
					strcpy(tBizData->opr_source			,oprSource	);
					printf("44444444444444444444444443读取用户的产品定购信息fDsmpGetOrderInfo: retCode=[%s]\n" ,retCode);

					/**	记录用户的产品定购明细信息 ***/
					sprintf(retCode, "%06d", fPubDsmpOrderMsg(tBizData,	temp_order_flag,retMsg));
					printf("记录用户的产品定购明细信息返回retCode=[%s]\n"	,retCode);
					if (strcmp(retCode,	"000000")	!= 0)
					{
						printf("fPubDsmpOrderMsg:retCode=[%s]	 retMsg=[%s] \n",	retCode,retMsg );
						EXEC SQL CLOSE vCurCancel;
					...
					printf("222333333333333333333333读取用户的产品定购信息fDsmpGetOrderInfo: retCode=[%s]\n" ,retCode);
			...
			/**	99-DSMP全业务退订	**/
			if ((strncmp(oprCode,BIZ_OPR_99,2) ==	0)	|| (strncmp(oprCode,BIZ_OPR_EXIT,2)	== 0 &&	(strcmp(tBizData->serv_code,"36")!=0)&&	(strcmp(tBizData->serv_code,"43")!=0)))/*add by shijing for 便民卡 at 20120131*/
			{
				strcpy(tBizData->id_no						,idNo	 );
				strcpy(tBizData->phone_no					,phoneNo );
				strcpy(tBizData->end_time					,opTime	);
		...
		#ifdef _DEBUG_
				printf("二次确认判断oprCode=[%s]\n", oprCode);
				printf("二次确认判断temp_reconfirm_flag=[%s]\n", temp_reconfirm_flag);
				printf("第三方确认判断temp_thirdvalidate=[%s]\n",	temp_thirdvalidate);
		#endif
